...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagRelay_On)
    {
        info->event[info->num] = EVENT_Relay_On;
        info->num++;
        dataPoints->valueRelay_On = gizStandardDecompressionValue(Relay_On_BYTEOFFSET,Relay_On_BITOFFSET,Relay_On_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagRelay_Off)
    {
        info->event[info->num] = EVENT_Relay_Off;
        info->num++;
        dataPoints->valueRelay_Off = gizStandardDecompressionValue(Relay_Off_BYTEOFFSET,Relay_Off_BITOFFSET,Relay_Off_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagKEY_UP)
    {
        info->event[info->num] = EVENT_KEY_UP;
        info->num++;
        dataPoints->valueKEY_UP = gizStandardDecompressionValue(KEY_UP_BYTEOFFSET,KEY_UP_BITOFFSET,KEY_UP_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagTIMER)
    {
        ...
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valueTIMER,issuedData->attrVals.valueTIMER,sizeof(issuedData->attrVals.valueTIMER));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueRelay_On != cur->valueRelay_On)
    {
        GIZWITS_LOG("valueRelay_On Changed\n");
        ret = 1;
    }
    if(last->valueRelay_Off != cur->valueRelay_Off)
    {
        GIZWITS_LOG("valueRelay_Off Changed\n");
        ret = 1;
    }
    if(last->valueKEY_UP != cur->valueKEY_UP)
    {
        GIZWITS_LOG("valueKEY_UP Changed\n");
        ret = 1;
    }
    if(0 != memcmp((uint8_t *)&last->valueTIMER,(uint8_t *)&cur->valueTIMER,sizeof(last->valueTIMER)))
    {
        GIZWITS_LOG("valueTIMER Changed\n");
        ret = 1;
    }
    ...
    if(last->valueu != cur->valueu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueu Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Relay_On_BYTEOFFSET,Relay_On_BITOFFSET,Relay_On_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRelay_On);
    gizStandardCompressValue(Relay_Off_BYTEOFFSET,Relay_Off_BITOFFSET,Relay_Off_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRelay_Off);
    gizStandardCompressValue(KEY_UP_BYTEOFFSET,KEY_UP_BITOFFSET,KEY_UP_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKEY_UP);
    ...
    devStatusPtr->valueu = exchangeBytes(gizY2XFloat(u_RATIO,  u_ADDITION, dataPoints->valueu));     



    gizMemcpy((uint8_t *)devStatusPtr->valueTIMER,(uint8_t *)&dataPoints->valueTIMER,sizeof(dataPoints->valueTIMER));
    return 0;
}
